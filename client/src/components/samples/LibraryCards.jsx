...
export default function LibraryCardsPage() {
  ...
  const filteredCards = libraryCards.filter((data) => {
    const matchesSearch =
      data.memberName.toLowerCase().includes(searchQuery.toLowerCase()) ||
      data.cardNumber.includes(searchQuery) ||
      data.memberId.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesFilter =
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 p-6">
            <div className="flex flex-col sm:flex-row gap-4">
              <div className="flex-1 relative">
                ...
                <input
                  type="text"
                  placeholder="Search by member name, data number, or member ID..."
                  value={searchQuery}
            ...
            >
              ...
              <div
                className={`bg-gradient-to-r from-${getStatusColor(
                  data.status
                )}-500 to-${getStatusColor(data.status)}-600 p-6 text-white`}
              >
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    ...
                    />
                    <div>
                      <h3 className="font-bold text-lg">{data.memberName}</h3>
                      <p className="text-white/80 text-sm">{data.memberId}</p>
                    </div>
                  </div>
                  <span
              ...
              <div className="p-6">
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-medium text-gray-900 dark:text-white">
                      {data.totalBooksIssued}
                    </span>
                  </div>
                  {data.finesOutstanding > 0 && (
                    <div className="flex items-center justify-between">
                      <span className="text-gray-600 dark:text-gray-400 text-sm">
                        Outstanding Fines
                      </span>
                      <span className="font-medium text-red-600 dark:text-red-400">
                        ${data.finesOutstanding}
                      </span>
                    </div>
                  )}
                  <div className="flex items-center justify-between">
        ...
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 p-6 text-center">
            ...
            <p className="text-2xl font-bold text-gray-900 dark:text-white">
              {libraryCards.reduce(
                (sum, data) => sum + data.totalBooksIssued,
                0
              )}
            </p>
            <p className="text-gray-600 dark:text-gray-400 text-sm">
