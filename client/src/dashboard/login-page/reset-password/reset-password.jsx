...
import {
  ...
  resetPasswordApplicantVerifyLink,
  resetPasswordStudent,
  resetPasswordStudentVerifyLink,
} from "../../http-requests";
...
const ResetPasswordPage = () => {
  ...
  useEffect(() => {
    const asyncFunc = async () => {
      switch (type) {
        case "applicant":
          await resetPasswordApplicantVerifyLink(type, code)
            .then((res) => {
        ...
        case "student":
          await resetPasswordStudentVerifyLink(type, code)
            .then((res) => {
              setFeedback([1, 1, res.message]);
              setIsLoading(false);
            })
        ...
        case "admin":
          await resetPasswordAdminVerifyLink(type, code)
            .then((res) => {
              setFeedback([1, 1, res.message]);
              setIsLoading(false);
            })
  ...
  const handleChangePassword = async () => {
    ...
    switch (type) {
      case "applicant":
        await resetPasswordApplicant(formFields.password, code)
          .then((res) => {
            setFeedback([1, 1, res.message]);
            navigate("/");
          })
      ...
      case "student":
        await resetPasswordStudent(formFields.password, code)
          .then((res) => {
            setFeedback([1, 1, res.message]);
            navigate("/");
          })
      ...
      {validLink ? (
        <div className="h-screen flex flex-row justify-center items-center">
          <div className="bg-white p-10 flex flex-col items-center gap-6 justify-center">
            ...
            <button className="my-button" onClick={handleChangePassword}>
              Confirm
            </button>
          </div>
        </div>
