...
const verifyEmailForLogin = async (req, res, next) => {
  try {
    const authStudentDoc = await getAuthMember({ email: req.body.email });
...
const createUserAndOTP = async (req, res, next) => {
  try {
    ...
    await createAuthMember(req.body, otp);
    const studentDoc = await getAuthMember({ email: req.body.email });
...
const processLink = async (req, res, next) => {
  try {
    await updateAuthMember(
      { _id: req.cust.authStudentDoc._id },
      {
...
const verifyResetLink = async (req, res, next) => {
  try {
    ...
    const authStudent = await getAuthMember({ resetCode: code });
